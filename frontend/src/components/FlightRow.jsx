...
export const FlightRow = ({ flight, onViewDetails, onEdit, onDelete }) => {
  ...
  return (
    <div className="apple-card p-4 sm:p-5 transition-colors duration-200 border border-[var(--border-subtle)] hover:border-[var(--border-hover)] w-full">
      <div className="hidden xl:flex items-center justify-between gap-6">
        ...
        <div className="flex-1 min-w-[360px] px-4">
          <div className="flex items-center justify-between gap-4">
            <div className="text-left">
              <p className="text-base font-bold font-display text-[var(--text-main)]">
                {flight.source}
              </p>
              <p className="text-xs font-mono text-[var(--text-sub)] flex items-center gap-1.5 mt-0.5 whitespace-nowrap">
                <Clock className="w-3.5 h-3.5 text-[var(--text-dim)] shrink-0" />
                <span className="font-semibold text-[var(--text-main)]">{dep.time}</span>
                <span className="text-[11px] text-[var(--text-dim)]">• {dep.date}</span>
              </p>
            </div>

            <div className="flex-1 max-w-[140px] flex items-center justify-center relative px-2">
            ...
            <div className="text-right">
              ...
              <p className="text-xs font-mono text-[var(--text-sub)] flex items-center justify-end gap-1.5 mt-0.5 whitespace-nowrap">
                <span className="text-[11px] text-[var(--text-dim)]">{arr.date} •</span>
        ...
        <div className="flex items-center space-x-1.5 pl-3 border-l border-[var(--border-subtle)] shrink-0">
          <button
      ...
      <div className="xl:hidden flex flex-col gap-3.5">
        <div className="flex items-center justify-between pb-3 border-b border-[var(--border-subtle)]">
          <div className="flex items-center space-x-2.5 min-w-0 pr-2">
            <span className="text-xs font-mono font-bold text-[var(--text-main)] bg-[var(--bg-pill)] px-2.5 py-1 rounded-xl border border-[var(--border-subtle)] shrink-0">
              #{flight.flightNumber}
            </span>
            <span className={`px-2.5 py-0.5 text-[8px] uppercase tracking-widest font-mono rounded-full shrink-0 ${getStatusStyle(flight.status)}`}>
              {(flight.status || 'SCHEDULED').replace('_', ' ')}
            </span>
            <span className="text-sm font-bold font-display text-[var(--text-main)] truncate">
              {flight.airlineName}
            </span>
          </div>
        ...
        <div className="flex items-center justify-between gap-3 sm:gap-6 py-1">
          <div className="flex-1 min-w-0 text-left">
            <p className="text-base sm:text-lg font-bold font-display text-[var(--text-main)] truncate">
              {flight.source}
            </p>
            <p className="text-xs font-mono text-[var(--text-sub)] flex items-center gap-1.5 mt-0.5 whitespace-nowrap">
              <Clock className="w-3.5 h-3.5 text-[var(--text-dim)] shrink-0" />
          ...
          <div className="w-16 sm:w-28 shrink-0 flex items-center justify-center relative px-2">
            <div className="w-full border-t border-dashed border-[var(--border-hover)]"></div>
            <AscendingPlaneIcon className="h-4 w-auto text-[var(--text-main)] absolute z-10 px-1 bg-[var(--bg-card-solid)] shrink-0" />
          </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-[var(--border-subtle)]">
          <div className="w-36 sm:w-44">
            <div className="flex items-center justify-between text-xs text-[var(--text-sub)] mb-1">
              <span className="text-[10px] font-mono text-[var(--text-dim)] uppercase">Seats Available</span>
              <span className="font-mono text-xs font-bold text-[var(--text-main)]">
                {flight.availableSeats}/{flight.totalSeats}
              </span>
            </div>
